...
void setup() {
  ...
  pinMode(PIN_LIMITSWITCH_2, INPUT_PULLUP);
...
void openDoorAction(int tme) {
  ...
  for (int i = 0; i < tme; i++) {
    ...
    if (digitalRead(PIN_LASERSENSOR) == LOW) {
      isOkay=false;
      break;
    }
  }
...
boolean isLimitSwitchActive(){
  //int isActive1=digitalRead(PIN_LIMITSWITCH_1);
  //int isActive2=digitalRead(PIN_LIMITSWITCH_2);

  if (digitalRead(PIN_LIMITSWITCH_1) == LOW || digitalRead(PIN_LIMITSWITCH_1) == LOW){
    return true;
  } else {
    return false;
  }
